...
<body>
    <div class="w-full">
        ...
        <nav id="navbar" class="fixed z-50 w-full top-0 start-0 px-4">
        ...
        $(document).ready(function() {
            $(window).on('scroll', function() {
                if ($(document).scrollTop() >= 10) {
                    ...
                    $('#navbar').removeClass('backdrop-blur-lg bg-white shadow-[0_2px_8px_rgb(0,0,0,0.3)]');
                    $('.nav-link').css('color', '#fff')
                    $('#logo img').attr('src', "{{ asset('assets/img/Logo Putih.png') }}");
                    $('.navbar-sticky').addClass('text-white');
                    $('.navbar-sticky').removeClass('text-black');
                }
            })
